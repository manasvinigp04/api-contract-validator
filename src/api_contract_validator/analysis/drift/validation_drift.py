...
class BehavioralDriftDetector:
    ...
    def detect(self, execution_summary: ExecutionSummary) -> List[BehavioralDriftIssue]:
        ...
        - Extra/missing fields that don't violate schema
    ...
    def _detect_structure_variations(
        ...
        """Detect variations in response structure (array vs object, nested depth, etc.)."""
        issues = []
    ...
    def _get_structure_signature(self, data: Any) -> str:
        ...
        Returns:
            String signature like "dict[str:str,id:int,items:list[dict]]"
        """
        if data is None:
            return "null"
            ...
            for key, value in sorted(data.items()):
                value_sig = self._get_structure_signature(value)
                fields.append(f"{key}:{value_sig}")
            return f"dict[{','.join(fields)}]"
        elif isinstance(data, list):
            if not data:
                return "list[empty]"
            # Sample first item for signature
            item_sig = self._get_structure_signature(data[0]) if data else "empty"
